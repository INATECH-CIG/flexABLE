        ...
        if t > self.Tprevious: # Müsste man noch anpassen, will ich gerade nicht
            if productionHour > 0: # Check if production in considered hour
                if startHour > 0: # Check if start of batch in considered hour
                    ...
                    if t == 53:# or t == 50 or t == 91:
                        print("flexSection before: ", flexSection)
                    ...
                    if t == 53:# or t == 50 or t == 91:
                        print("flexSection after: ", flexSection)                    
                    ...
                    if t == 53:#or t == 50 or t == 91:
                        self.resultsSegment_all.to_csv(f'output/2016example/SteelPlant/prod_opt_{t}_before.csv', index=True)
                        SectionModelFlex[0].to_csv(f'output/2016example/SteelPlant/flex_opt_{t}.csv', index=True)
                        # Das ist quasi das Feedback, das muss noch anders eingebaut werden, soll nur passieren, wenn flex gebote akzeptiert
                        print("timestampsSectionFlex: ", timestampsSectionFlex)
                        self.resultsSegment_all.iloc[t:t+timestampsSectionFlex+1] = SectionModelFlex[0][-(timestampsSectionFlex+1):].reset_index(drop=True)
                        print(self.resultsSegment_all)
                    if t == 53:# or t == 50 or t == 91:
                        self.resultsSegment_all.to_csv(f'output/2016example/SteelPlant/prod_opt_{t}_after.csv', index=True)
