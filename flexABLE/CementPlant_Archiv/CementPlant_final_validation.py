...
class CementPlant():
    ...
    def __init__(self,
        ...
        el_PFC_df = pd.DataFrame(self.dicPFC)
        el_PFC_df.to_csv('output/2016example/CementPlant/PFC.csv', index=True)
    ...
    def feedback(self, bid):
        if bid.status == "Confirmed":
            ...
            if round(self.confQtyEOM[self.world.currstep],2) != round(self.resultsSegment_all.iloc[self.world.currstep]["el_cons_total"],2):
                ...
                test_df = SectionModelFlex
                test_df.to_csv(f'output/2016example/CementPlant/SectionModelFlex_{self.world.currstep}.csv', index=True)
    ...
    def requestBid(self, t, market):
        ...
        if market == "EOM":
            ...
            if BidsDict['bidQuantity_flex'] == 0:
                ...
                print(t, "Flex bid: ", BidsDict['bidQuantity_flex'], BidsDict['bidPrice_flex'])
                bids.append(Bid(issuer = self, 
    ...
    def calculateFlexBids(self, t): 
        ...
        if t > 16 and self.resultsSegment_all["clinker_dome"][t] + self.minPowerKiln <= self.maxCapClinkerDome and self.resultsSegment_all["cement_mill_start"][t-16:t].sum() == 0 and self.resultsSegment_all["cement_mill_on"][t] == 1:
            CementMillFlex = True
            # if self.resultsSegment_all["cement_mill_stop"][t-3:t].sum() == 1 and self.resultsSegment_all["output_cement_mill"][t-3:t].sum() == 0: # zusätzliche Bedinung, da Änderung des shutdown_change(t+1) nicht möglich und das dann zu Problemen bei der Optimierung führt --> Unschön könnte man noch in die Optimierung mit reinnehmen, keine Zeit #HERE muss ich auch noch für RawMill machen
            #     CementMillFlex = False
            #     print(t, "CementMillFlex not possible")
        else:
            CementMillFlex = False
        ...
        if CementMillFlex == True and RawMillFlex == True:
            ...
            print(t,"productionCostSegmentOld" ,productionCostSegmentOld)
            print(t, "productionCostSegmentNew",productionCostSegementNew)
            print(t, "objective new:", SectionModelFlex_RawMill_CementMill[1])
            ...
            if RawMillFlex == True:
                ...
                print("Überprüfung:", startFlex, t + timestampsSectionFlex)
                print("Überprüfung_len SectionModelFlex:", len(SectionModelFlex))
                ...
                print(t,"productionCostSegmentOld" ,productionCostSegmentOld)
                print(t, "productionCostSegmentNew",productionCostSegementNew)
                print(t, "objective new:", SectionModelFlex_RawMill[1])
                ...
                if CementMillFlex == True:
                    ...
                    # if flexSection["cement_mill_stop"].iloc[-5:-1].sum() == 1 and flexSection["cement_mill_shut_down"].iloc[-5:-1].sum() == 5:
                    #     flexSection["cement_mill_shut_down"][flexSection.index[-1]] = 0
                    #     flexSection["cement_mill_shut_down_change"][flexSection.index[-1]] = 0
                    # else:
                    #     flexSection["cement_mill_shut_down_change"][flexSection.index[-1]] = 1
                    # if flexSection["cement_mill_start"][flexSection.index[-1]] == 1:
                    #     flexSection["cement_mill_stop"][flexSection.index[-1]] = 0
                    #     if flexSection["cement_mill_shut_down"].iloc[-4:-1].sum() == 0:
                    #         flexSection["cement_mill_shut_down"][flexSection.index[-1]] = 0
                    #     else:
                    #         flexSection["cement_mill_shut_down"][flexSection.index[-1]] = 1
                    # else:
                ...
                # benötigt man, beispiel: t = 26, timestampsPreviosuSection = 24 --> untere Grenze des PFC abrufs wird 2, aber t previous geht eigentlich von 3-26 (24 einheiten)
                    ...
                    print(t,"productionCostSegmentOld" ,productionCostSegmentOld)
                    print(t, "productionCostSegmentNew",productionCostSegementNew)
                    print(t, "objective new:", SectionModelFlex_CementMill[1])
        ...
        # if CementMillFlex == True: # (everythin after before clinker dome shouldn't be changed)
        ...
        #     SectionModelFlex = self.cementOptBase(
        #         optHorizon = timestampsSectionFlex, 
        #         timestampsPreviousSection = timestampsPreviousSectionFlex, 
        #         PFC = [abs(value) for value in self.dicPFC[(t-timestampsPreviousSectionFlex):(t+timestampsSectionFlex)]], #erstmal nur positive um Überprodukion zu verhindern
        #         previousSection = flexSection,
        #         productionGoal = sum(self.resultsSegment_all["output_cement_mill"][startFlex:t+timestampsSectionFlex]), 
        #         minPowerRawMill = self.minPowerRawMill,
        #         maxPowerRawMill = self.maxPowerRawMill,
        #         minPowerKiln = self.minPowerKiln,
        #         maxPowerKiln = self.maxPowerKiln,
        ...
        #     productionCostSegmentOld = sum([self.resultsSegment_all["el_cons_total"].iloc[i] * abs(self.dicPFC[i-1]) for i in range(startFlex, t + timestampsSectionFlex)])
        #     productionCostSegementNew = sum([SectionModelFlex[0]["el_cons_total"].iloc[i] * abs(self.dicPFC[i-1]) for i in range(1, len(SectionModelFlex[0]))])
        ...
        # 'if RawMillFlex == True: # (everythin after raw meal silo shouldn't be changed)
        #     flexAmountI = int(flexSection["el_cons_raw_mill"][flexSection.index[-1]])
            
        #     flexSection["raw_meal_silo"][flexSection.index[-1]] = flexSection["raw_meal_silo"][flexSection.index[-1]] - flexSection["output_raw_mill"][flexSection.index[-1]]
        #     flexSection["input_raw_mill"][flexSection.index[-1]] = 0
        #     flexSection["output_raw_mill"][flexSection.index[-1]] = 0
        #     flexSection["el_cons_raw_mill"][flexSection.index[-1]] = 0
        #     flexSection["raw_mill_on"][flexSection.index[-1]] = 0
        #     if flexSection["raw_mill_stop"].iloc[-4:-1].sum() == 1:
