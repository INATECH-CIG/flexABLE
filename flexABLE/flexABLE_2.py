...
class World():
    ...
    def loadScenario(self,
        ...
        if networkEnabled:
            ...
            buses = pd.read_csv('input/{}/nodes.csv'.format(scenario),
                                          encoding = "Latin-1",
                                          dtype={'name':str})
            buses.set_index('name', inplace=True)
            demand_dist = pd.read_csv('input/{}/IED_dist.csv'.format(scenario),
                                 nrows =len(self.snapshots) + startingPoint, index_col=0)
            ...
            solar_dist = pd.read_csv('input/{}/solar_dist.csv'.format(scenario),
                                 nrows  =len(self.snapshots) + startingPoint,
                                 index_col = 0)
            solar_dist.drop(solar_dist.index[0:startingPoint], inplace = True)
            solar_dist.reset_index(drop = True, inplace = True)
            
            wind_dist = pd.read_csv('input/{}/wind_dist.csv'.format(scenario),
                                 nrows  =len(self.snapshots) + startingPoint,
                                 index_col = 0)
            ...
            self.network.madd('Load',
                              demand_dist.columns,
                              suffix='_load',
                              bus=demand_dist.columns,
                              p_set = pd.DataFrame(0, index=self.network.snapshots, columns=demand_dist.columns))
            self.network.madd('Generator', buses[buses['PV']>0].index,
                              suffix='_PV_mrEOM',
                              bus=buses[buses['PV']>0].index,
                              carrier='PV',
                              p_nom=1,
                              p_max_pu=1,
                              p_min_pu=1,
            ...
            self.network.madd('Generator', buses[buses['PV']>0].index,
                              ...
                              p_min_pu=0,
                              marginal_cost=0)
            self.network.madd('Generator', buses[(buses['windOn']+buses['windOff'])>0].index,
                              suffix='_Wind_mrEOM',
                              bus=buses[(buses['windOn']+buses['windOff'])>0].index,
                              carrier='Wind',
                              p_nom=1,
                              p_max_pu=1,
                              p_min_pu=1,
            ...
            self.network.madd('Generator', buses[(buses['windOn']+buses['windOff'])>0].index,
                              ...
                              p_min_pu=0,
                              marginal_cost=0)
            # add backup and load shedding generators to all nodes
            self.network.madd('Generator', buses.index,
                              suffix='_backup',
                              bus=buses.index,
                              carrier='backup',
                              p_nom=50000,
                              ...
                              marginal_cost=30000)
            self.network.madd('Generator', buses.index,
                                suffix='_loadshedding',
                                bus=buses.index,
                                carrier='loadshedding',
                                sign=-1,
                                p_nom=50000,
                                p_max_pu=1,
                                p_min_pu=0,
            ...
            for name, info in buses.iterrows():
                if info['PV']>0:
                    self.agents['Renewables'].addVREPowerplant('{}_PV'.format(name),
                                                               FeedInTimeseries = (solar_dist[str(name)]*info['PV']).to_list())
                if info['windOn']+info['windOff']>0:
                    self.agents['Renewables'].addVREPowerplant('{}_Wind'.format(name),
                                                               FeedInTimeseries = (wind_dist[name]*(info['windOn']+info['windOff'])).to_list())
            lines = pd.read_csv('input/{}/Lines.csv'.format(scenario),
                                encoding = "Latin-1", dtype={'name':str,
                                                             'bus0':str,
                                                             'bus1':str})
            links = pd.read_csv('input/{}/Links.csv'.format(scenario), index_col=0, encoding="Latin-1")

            #lines.set_index('name', inplace=True)
            self.network.madd('Line', lines.index,
                              bus0=lines['bus0'],
                              bus1=lines['bus1'],
                              x=lines['x'],
                              r=lines['r'],
                              s_nom=lines['s_nom']*lines['Circuits'])
            self.network.madd('Link', links.index,
                              bus0=links['bus0'],
                              bus1=links['bus1'],
                              p_nom=links['p_nom'])
        ...
        powerplantsList = pd.read_csv('input/{}/FPP_DE.csv'.format(scenario),
                                      ...
                                      encoding = "Latin-1")
        if networkEnabled:
            self.network.madd('Generator', powerplantsList.index,
                              suffix='_mrEOM',
            ...
            self.network.madd('Generator', powerplantsList.index,
                              ...
                              marginal_cost=0)
            self.network.madd('Generator', powerplantsList.index,
                              suffix='_mrEOM_negative',
                              bus=powerplantsList.node,
                              carrier='negative_redispatch',
                              p_nom=1,
                              sign=1,
                              p_max_pu=1,
                              p_min_pu=0,
            ...
            self.network.madd('Generator', powerplantsList.index,
                              ...
                              marginal_cost=0)
            self.network.madd('Generator', powerplantsList.index,
                              suffix='_mrEOM_positive',
                              bus=powerplantsList.node,
                              carrier='positive_redispatch',
                              p_nom=1,
                              sign=1,
                              p_max_pu=1,
                              p_min_pu=0,
