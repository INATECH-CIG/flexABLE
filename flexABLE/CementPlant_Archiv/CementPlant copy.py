...
class CementPlant():
    ...
    def __init__(self,
        ...
        print("PFC: ", df_PFC)
        df_PFC.to_csv(f'output/2016example/CementPlant/PFC.csv', index=False)
    ...
    def feedback(self, bid):
        if bid.status == "Confirmed":
            ...
            # if round(self.confQtyEOM[self.world.currstep],2) != round(self.resultsSegment_all.iloc[self.world.currstep]["el_cons_total"],2):
                ...
                # self.resultsSegment_all.to_csv(f'output/2016example/CementPlant/prod_opt_{self.world.currstep}_before.csv', index=True) #HERE
    ...
    def requestBid(self, t, market):
        ...
        if market == "EOM":
            ...
            # bids.append(Bid(issuer = self, #HERE
    ...
    def calculateBaseline(self, t):
        ...
        if t == 0:
                ...
                # 2. calculate opt production for each segment #HERE
    ...
    def calculateFlexBids(self, t): #HERE

        global SectionModelFlex
        global timestampsSectionFlex
        
        # check if raw_mill can be turned off
        if self.resultsSegement_all["raw_meal_silo"][t] >= self.maxPowerKiln/self.rawMillClinkerFactor and self.resultsSegment_all["raw_mill_start"][t-16:t].sum() == 0 and self.resultsSegment_all["raw_mill_on"][t] == 1:
            RawMillFlex = True
            print(t, "RawMillFlex: ", RawMillFlex)
        ...
        if t > self.Tprevious: # Müsste man noch anpassen, will ich gerade nicht ### Check ###
            if productionHour > 0: # Check if production in considered hour
                if startHour > 0: # Check if start of batch in considered hour
                    ...
                    flexPriceI = productionCostSegementNew - productionCostSegmentOld 
    ...
    def calculateBidEOM(self, t):
        ...
        # BidsEOM['bidQuantity_flex'], BidsEOM['bidPrice_flex'] = self.calculateFlexBids(t) #HERE
